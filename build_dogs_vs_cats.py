...
# Serialize all images in the data split to the HDF5 dataset

trainPaths = list(paths.list_images(config.IMAGES_PATH))
# sample data image path Dogs-vs-Cats/cat.1.jpg
trainLabels = [p.split(os.path.sep)[-1].split(".")[0] for p in trainPaths]

le = LabelEncoder()
trainLabels = le.fit_transform(trainLabels)

# Three splits: a training split, a validation split, testing split
...
for (dType, paths, labels, outputPath) in datasets:
    ...
    for (i, (path, label)) in enumerate(zip(paths, labels)):
        ...
        # compute the mean of each channel in the image for the training dataset
        if dType == "train":
